...
public interface RecipeStepRepository extends JpaRepository<RecipeStep, UUID> {
    ...
    @Query(value = """
        SELECT new com.backend.cookshare.recipe_management.dto.response.RecipeStepResponse(
            rs.stepNumber,
            rs.instruction,
            rs.imageUrl,
            rs.videoUrl,
    ...
    @Query(value = "SELECT step_number, image_url FROM recipe_steps WHERE recipe_id = :recipeId", nativeQuery = true)
    List<Map<String, Object>> findStepImagesByRecipeId(@Param("recipeId") UUID recipeId);
}
