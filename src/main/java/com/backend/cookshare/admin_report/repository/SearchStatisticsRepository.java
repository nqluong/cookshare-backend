...
public interface SearchStatisticsRepository extends JpaRepository<SearchHistory, UUID> {
     ...
     */
    @Query("""
            SELECT COUNT(sh) FROM SearchHistory sh
     ...
     */
    @Query("""
            SELECT COUNT(DISTINCT sh.searchQuery) FROM SearchHistory sh
            WHERE sh.createdAt BETWEEN :startDate AND :endDate
            """)
     ...
     */
    @Query("""
            SELECT COUNT(DISTINCT sh.userId) FROM SearchHistory sh
            WHERE sh.createdAt BETWEEN :startDate AND :endDate
     ...
     */
    @Query("""
            SELECT AVG(sh.resultCount) FROM SearchHistory sh
            WHERE sh.createdAt BETWEEN :startDate AND :endDate
            """)
